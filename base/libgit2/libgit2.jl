...
"""
    reset!(repo::GitRepo, commit::GitHash, mode::Cint = Consts.RESET_MIXED)

Reset the repository `repo` to its state at `commit`, using one of three modes
set by `mode`:
  1. `Consts.RESET_SOFT` - move HEAD to `commit`.
  2. `Consts.RESET_MIXED` - default, move HEAD to `commit` and reset the index to `commit`.
  3. `Consts.RESET_HARD` - move HEAD to `commit`, reset the index to `commit`, and discard all working changes.
